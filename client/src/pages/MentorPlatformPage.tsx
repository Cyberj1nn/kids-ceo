...
/* ============================================================
   Контент-данные (для удобной правки)
   ============================================================ */
const PAINS = [
...
/* ============================================================
   Хук скролла для sticky-CTA и hover-навбара
   ============================================================ */
...
function useReveal() {
  ...
  useEffect(() => {
    ...
    const targets = ref.current.querySelectorAll<HTMLElement>('.mp-reveal');
    if (targets.length === 0) return;
    if (typeof IntersectionObserver === 'undefined') {
      targets.forEach((el) => el.classList.add('mp-reveal--visible'));
      return;
    }
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            e.target.classList.add('mp-reveal--visible');
...
/* ============================================================
   Page
   ============================================================ */
export default function MentorPlatformPage() {
  ...
  return (
    <div className="mp" ref={rootRef}>
      ...
      {/* ---- NAV ---- */}
      <header className={`mp-nav${scrolled ? ' mp-nav--scrolled' : ''}`}>
        <div className="mp__wrap mp-nav__inner">
          <a className="mp-nav__logo" href="#top" aria-label="К началу страницы">
            <span className="mp-nav__logo-mark">M</span>
            <span>Mentor Platform</span>
          </a>
          <nav className="mp-nav__menu" aria-label="Разделы лендинга">
            <a href="#features">Возможности</a>
            <a href="#case">Кейс</a>
            <a href="#pricing">Стоимость</a>
            <a href="#faq">FAQ</a>
          </nav>
          <a className="mp-btn mp-btn--primary mp-nav__cta" href="#cta">Получить разбор</a>
        </div>
      </header>
      ...
      <main id="top">
        ...
        <section className="mp-hero">
          <div className="mp__wrap mp-hero__grid">
            <div className="mp-hero__content mp-reveal">
              <span className="mp-eyebrow mp-hero__eyebrow">Разработка обучающих платформ под ключ</span>
              <h1 className="mp-h1 mp-hero__h1">
                Своя платформа для обучения команды и учеников — от&nbsp;15&nbsp;000&nbsp;₽
              </h1>
              <p className="mp-lead mp-hero__lead">
              ...
              <div className="mp-hero__cta">
                <a className="mp-btn mp-btn--primary mp-btn--lg" href="#cta">
                  Получить бесплатный разбор {I.arrow}
                </a>
                <a className="mp-btn mp-btn--secondary mp-btn--lg" href="#case">
                  Посмотреть кейс Kids CEO
                </a>
              </div>
              <ul className="mp-hero__trust">
                <li className="mp-hero__trust-item">
                  <span className="mp-hero__trust-ico" aria-hidden="true">{I.check}</span>
            ...
            <div className="mp-hero__visual mp-reveal" aria-hidden="true">
              <div className="mp-hero__device">
                <div className="mp-hero__device-bar">
                  <span /><span /><span />
                </div>
                <div className="mp-hero__device-rows">
                  <div className="mp-hero__device-row mp-hero__device-row--brand" />
                  <div className="mp-hero__device-row" />
                  <div className="mp-hero__device-row mp-hero__device-row--short" />
                </div>
                <div className="mp-hero__device-card">
                  <div className="mp-hero__device-row" />
                  <div className="mp-hero__device-row mp-hero__device-row--short" />
                </div>
                <div className="mp-hero__device-card">
        ...
        {/* ---- PAIN ---- */}
        <section className="mp-section mp-section--warm">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Проблема</span>
              <h2 className="mp-h2">Если что-то из этого знакомо — пора менять подход</h2>
            </div>
            <div className="mp-pain__list">
              {PAINS.map((p, i) => (
                <div className="mp-card mp-reveal" key={i}>
                  <span className="mp-card__num">0{i + 1}</span>
                  <h3 className="mp-card__title">{p.title}</h3>
                  <p className="mp-card__text">{p.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mp-section">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Решение</span>
              <h2 className="mp-h2">Ваша собственная платформа со всеми инструментами в одном месте</h2>
              <p className="mp-lead" style={{ marginTop: 16 }}>
            ...
            <div className="mp-solution__pillars">
              {PILLARS.map((p, i) => (
                <div className="mp-card mp-reveal" key={i}>
                  <div className="mp-card__ico" aria-hidden="true">{p.ico}</div>
                  <h3 className="mp-card__title">{p.title}</h3>
                  <p className="mp-card__text">{p.text}</p>
                </div>
              ))}
        ...
        <section className="mp-section mp-section--warm">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Кому подходит</span>
              <h2 className="mp-h2">Для кого эта платформа</h2>
            </div>
            <div className="mp-audience__cards">
              {AUDIENCE.map((a, i) => (
                <div className="mp-audience__card mp-reveal" key={i}>
                  <span className="mp-audience__tag">{a.tag}</span>
                  <h3 className="mp-audience__title">{a.title}</h3>
                  <p className="mp-audience__text">{a.text}</p>
                  <p className="mp-audience__what"><strong>Что получаете:</strong> {a.what}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mp-section" id="features">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Функционал</span>
              <h2 className="mp-h2">Что входит в платформу</h2>
              <p className="mp-lead" style={{ marginTop: 16 }}>
                Базовая комплектация — на основе уже работающего проекта Kids&nbsp;CEO. Расширения подбираем под ваши задачи.
              </p>
            </div>

            <div className="mp-features__base">
              {FEATURES_BASE.map((f, i) => (
                <div className="mp-feature mp-reveal" key={i}>
                  {f.star && <span className="mp-feature__star">Ключевой модуль</span>}
                  <div className="mp-feature__head">
                    <div className="mp-feature__ico" aria-hidden="true">{f.ico}</div>
                    <h3 className="mp-feature__title">{f.title}</h3>
                  </div>
                  <p className="mp-feature__text">{f.text}</p>
                </div>
              ))}
            </div>

            <div className="mp-features__divider mp-reveal">
              <span className="mp-features__divider-text">Расширения по запросу</span>
            </div>

            <div className="mp-features__ext">
              {FEATURES_EXT.map((e, i) => (
                <div className="mp-features__ext-item mp-reveal" key={i}>
                  <span className="mp-features__ext-ico" aria-hidden="true">{I.plus}</span>
                  <span>{e}</span>
                </div>
              ))}
            </div>
        ...
        {/* ---- CASE ---- */}
        <section className="mp-section mp-section--dark" id="case">
          <div className="mp__wrap">
            <div className="mp-case__inner">
              <div className="mp-case__text mp-reveal">
                <span className="mp-eyebrow" style={{ color: 'var(--brand)' }}>Кейс</span>
                <h2 className="mp-h2 mp-case__h2">Реальный кейс — платформа, которая уже работает</h2>

                <div className="mp-case__row">
                  <div className="mp-case__row-label">Что было</div>
                  <p className="mp-case__row-text">Онлайн-школа управленцев бизнеса (детские центры) обучала руководителей через Telegram, Google Drive и Zoom. Контент дублировался, ученики путались, наставники не успевали отслеживать прогресс каждого.</p>
                </div>

                <div className="mp-case__row">
                  <div className="mp-case__row-label">Что сделали</div>
                  <p className="mp-case__row-text">Полноценное веб-приложение с <strong>11 разделами</strong>, чатами в реальном времени, таблицей ДТП с историей правок, трекером созвонов, ролевой моделью (суперадмин / админ / наставник / ученик), приёмом оплат и мобильной PWA-версией.</p>
                </div>

                <div className="mp-case__row">
                  <div className="mp-case__row-label">Стек</div>
                  <p className="mp-case__row-text">React + TypeScript, Node.js + Express, PostgreSQL, Socket.io, JWT-авторизация. Развёрнуто на собственном сервере под доменом kids-ceo.ru.</p>
                </div>

                <div className="mp-case__highlight">
              ...
              <div className="mp-case__media mp-reveal">
                <div className="mp-case__media-fallback">
                  Скриншот платформы Kids CEO
                </div>
                <img
                  className="mp-case__img"
                  ...
                  loading="lazy"
                  decoding="async"
                  onError={(e) => { (e.currentTarget as HTMLImageElement).classList.add('mp-case__img--hidden'); }}
                />
              </div>
            </div>
          </div>
        ...
        <section className="mp-section">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Отстройка</span>
              <h2 className="mp-h2">Сравнение: наша платформа vs альтернативы</h2>
            </div>

            {/* Mobile */}
            <div className="mp-compare__cards">
              <div className="mp-compare__col mp-reveal">
                <div className="mp-compare__title">GetCourse / Kwiga</div>
                {COMPARE_ROWS.map((r, i) => (
                  <div className="mp-compare__row" key={i}>
                    <span className="mp-compare__row-label">{r.label}</span>
                    <span className="mp-compare__row-value">{r.getCourse}</span>
                  </div>
                ))}
              </div>
              <div className="mp-compare__col mp-reveal">
                <div className="mp-compare__title">Конструкторы (Tilda)</div>
                {COMPARE_ROWS.map((r, i) => (
                  <div className="mp-compare__row" key={i}>
                    <span className="mp-compare__row-label">{r.label}</span>
                    <span className="mp-compare__row-value">{r.tilda}</span>
                  </div>
                ))}
              </div>
              <div className="mp-compare__col mp-reveal">
                <div className="mp-compare__title">Разработка с нуля</div>
                {COMPARE_ROWS.map((r, i) => (
                  <div className="mp-compare__row" key={i}>
                    <span className="mp-compare__row-label">{r.label}</span>
                    <span className="mp-compare__row-value">{r.custom}</span>
                  </div>
                ))}
              </div>
              <div className="mp-compare__col mp-compare__col--us mp-reveal">
                <div className="mp-compare__title">Наша платформа</div>
                {COMPARE_ROWS.map((r, i) => (
                  <div className="mp-compare__row" key={i}>
                    <span className="mp-compare__row-label">{r.label}</span>
                    <span className="mp-compare__row-value">{r.us}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Desktop */}
            <table className="mp-compare__table mp-reveal">
              <thead>
                <tr>
                  <th></th>
                  ...
                  <th className="mp-compare__us">Наша платформа</th>
                </tr>
              </thead>
              <tbody>
                {COMPARE_ROWS.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td>{r.tilda}</td>
                    <td>{r.custom}</td>
                    <td className="mp-compare__us">{r.us}</td>
                  </tr>
                ))}
              </tbody>
            </table>

            <div className="mp-compare__highlight mp-reveal">
        ...
        <section className="mp-section mp-section--warm">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Этапы работы</span>
              <h2 className="mp-h2">Как мы работаем</h2>
            </div>
            <div className="mp-process__steps">
              {PROCESS.map((p, i) => (
                <div className="mp-process__step mp-reveal" key={i}>
                  <div className="mp-process__num">{p.num}</div>
                  <div>
                    <span className="mp-process__step-time">{p.time}</span>
                    <h3 className="mp-process__step-title" style={{ marginTop: 4 }}>{p.title}</h3>
                  </div>
                  <p className="mp-process__step-text">{p.text}</p>
                </div>
              ))}
            </div>
            <div className="mp-process__after mp-reveal">
        ...
        {/* ---- GET LIST ---- */}
        <section className="mp-section mp-section--sand">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">При запуске</span>
              <h2 className="mp-h2">В день запуска вы получаете</h2>
            </div>
            <ul className="mp-getlist__list">
              {GET_LIST.map((item, i) => (
                <li className="mp-getlist__item mp-reveal" key={i}>
                  <span className="mp-getlist__check" aria-hidden="true">{I.check}</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        <section className="mp-section">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Технологии</span>
              <h2 className="mp-h2">На чём построено</h2>
              <p className="mp-lead" style={{ marginTop: 16 }}>
                Современный, проверенный стек. Любой Frontend / Backend разработчик с рынка сможет подхватить проект.
              </p>
            </div>
            <div className="mp-stack__grid">
              {STACK.map((s, i) => (
                <div className="mp-stack__card mp-reveal" key={i}>
                  <div className="mp-stack__title">{s.title}</div>
                  <ul className="mp-stack__list">
                    {s.items.map((it, j) => <li key={j}>{it}</li>)}
                  </ul>
                </div>
              ))}
            </div>
            <div className="mp-stack__note mp-reveal">
        ...
        {/* ---- PRICING ---- */}
        <section className="mp-section mp-section--warm" id="pricing">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">Стоимость</span>
              <h2 className="mp-h2">Прозрачные цены без скрытых платежей</h2>
              <p className="mp-lead" style={{ marginTop: 16 }}>
                Платите за развёртывание один раз — дальше только ежемесячное сопровождение.
              </p>
            </div>

            <div className="mp-pricing__grid">
              <div className="mp-pricing__card mp-reveal">
                <div className="mp-pricing__name">Базовый функционал</div>
                <div>
                  <div className="mp-pricing__price">15 000 ₽</div>
                  <div className="mp-pricing__price-note">разово, развёртывание</div>
                </div>
                <p className="mp-pricing__desc">Готовая платформа на базе Kids CEO под ваш бренд.</p>
                <ul className="mp-pricing__list">
                  {[
                    ...
                    <li className="mp-pricing__li" key={i}>
                      <span className="mp-pricing__check" aria-hidden="true">{I.check}</span>
                      <span>{t}</span>
                    </li>
                  ))}
                </ul>
                <div className="mp-pricing__time">Срок запуска: <strong>1–2 недели</strong></div>
                <a className="mp-btn mp-btn--secondary mp-btn--block" href="#cta">Заказать платформу</a>
              </div>

              <div className="mp-pricing__card mp-pricing__card--featured mp-reveal">
                <span className="mp-pricing__featured-tag">Гибкое решение</span>
                <div className="mp-pricing__name">Индивидуальный запрос</div>
                <div>
                  <div className="mp-pricing__price">20–25 000 ₽</div>
                  <div className="mp-pricing__price-note">за каждый кастомный модуль</div>
                </div>
                <p className="mp-pricing__desc">Базовый функционал + ваши уникальные доработки.</p>
                <ul className="mp-pricing__list">
                  {[
                ...
                <div className="mp-pricing__time">Срок запуска: <strong>2–3 недели</strong></div>
                <a className="mp-btn mp-btn--primary mp-btn--block" href="#cta">Обсудить задачу</a>
              </div>
            </div>

            <div className="mp-pricing__support mp-reveal">
              <div className="mp-pricing__support-head">
                <div className="mp-pricing__support-tag">Обслуживание и поддержка</div>
                <div className="mp-pricing__support-price">3 500 ₽/мес</div>
              </div>
              <ul className="mp-pricing__support-list">
        ...
        {/* ---- FAQ ---- */}
        <section className="mp-section" id="faq">
          <div className="mp__wrap">
            <div className="mp-section__head mp-reveal">
              <span className="mp-eyebrow">FAQ</span>
              <h2 className="mp-h2">Частые вопросы</h2>
            </div>
            <div className="mp-faq__list">
              {FAQ.map((item, i) => (
                <details className="mp-faq__item mp-reveal" key={i}>
                  <summary className="mp-faq__q">
                    <span>{item.q}</span>
                    <span className="mp-faq__q-ico" aria-hidden="true">{I.plus}</span>
                  </summary>
                  <div className="mp-faq__a"><p>{item.a}</p></div>
                </details>
              ))}
            </div>
          </div>
        ...
        {/* ---- FINAL CTA ---- */}
        <section className="mp-section mp-section--sand" id="cta">
          <div className="mp__wrap">
            <div className="mp-cta__head mp-reveal">
              <span className="mp-eyebrow">Связаться</span>
              <h2 className="mp-h2" style={{ marginTop: 12 }}>Получите бесплатный разбор вашего проекта</h2>
              <p className="mp-lead" style={{ marginTop: 16 }}>
            ...
            <div className="mp-cta__channels">
              <a
                className="mp-cta__channel mp-cta__channel--primary mp-reveal"
                href="https://t.me/Cyberj1nn"
                target="_blank"
                rel="noopener noreferrer"
              >
                <span className="mp-cta__channel-ico" aria-hidden="true">{I.telegram}</span>
                <span className="mp-cta__channel-title">Написать в Telegram</span>
                <span className="mp-cta__channel-handle">@Cyberj1nn</span>
              </a>

              <a
                className="mp-cta__channel mp-reveal"
                href="https://wa.me/79278060278"
                target="_blank"
                rel="noopener noreferrer"
              >
                <span className="mp-cta__channel-ico" aria-hidden="true">{I.whatsapp}</span>
                <span className="mp-cta__channel-title">Написать в WhatsApp</span>
                <span className="mp-cta__channel-handle">+7 927 806-02-78</span>
              </a>
              ...
              >
                <span className="mp-cta__channel-ico" aria-hidden="true">{I.mail}</span>
                <span className="mp-cta__channel-title">Написать на почту</span>
                <span className="mp-cta__channel-handle">fesha.lucky@gmail.com</span>
              </a>
            </div>

            <p className="mp-cta__note">Отвечаем в течение 1–2 часов в рабочее время (МСК).</p>
          </div>
        </section>
            ...
            <div className="mp-footer__grid">
              ...
              <div>
                <div className="mp-footer__brand">
                  <span className="mp-footer__brand-mark">M</span>
                  Sochi Sites
                </div>
                <div className="mp-footer__col-title">Студия</div>
              ...
              <div>
                <div className="mp-footer__col-title">Контакты</div>
                <ul className="mp-footer__list">
              ...
              <div>
                <div className="mp-footer__col-title">Реквизиты</div>
      ...
      {/* ---- STICKY MOBILE CTA ---- */}
      <div className={`mp-sticky-cta${showSticky ? ' mp-sticky-cta--shown' : ''}`}>
        <a className="mp-btn mp-btn--primary" href="#cta">Получить разбор</a>
      </div>
