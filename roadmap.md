...
### 6.3. Принудительная смена пароля
- [x] Миграция `018_must_change_password.sql` — флаг `users.must_change_password`
- [x] Флаг ставится в `true` при создании пользователя админом и при сбросе пароля
- [x] `/login` и `/me` возвращают `mustChangePassword`
- [x] `PUT /api/auth/password` — самостоятельная смена пароля (валидация текущего)
- [x] При `mustChangePassword=true` — редирект на `/change-password` (PrivateRoute)
- [x] Страница `ChangePasswordPage` — экран смены (текущий + новый + повтор)
- [x] Кнопка "Сменить пароль" в шапке для добровольной смены
...
## Фаза 10. Группы пользователей и адресная видимость событий

### 10.1. Группы пользователей
- [x] Миграция `016_groups_and_audience.sql` — таблицы `user_groups` (мягкое удаление), `user_group_members` (composite PK)
- [x] Уникальность названия группы (case-insensitive) среди активных
- [x] Backend `routes/groups.ts` — CRUD групп (admin/mentor/superadmin) + GET/PUT членов
- [x] Backend `routes/users.ts` — GET `/api/users/:id/groups`, PUT `/api/users/:id/groups` для назначения групп пользователю
- [x] Frontend `api/groups.ts` + новая вкладка "Группы" в `AdminPage` (`components/GroupsAdmin.tsx`)
- [x] Создание/переименование/мягкое удаление групп с подтверждением
- [x] Управление участниками: модалка с поиском по имени/логину, чекбоксы, bulk-сохранение
- [x] В форме пользователя (UserForm в AdminPage) — чекбоксы групп при создании и редактировании

### 10.2. Адресная видимость событий календаря
- [x] Колонка `audience_type` ENUM('all', 'users', 'groups') на `calendar_events`
- [x] Junction-таблицы `calendar_event_users`, `calendar_event_groups`
- [x] GET `/api/calendar/events` фильтрует по audience: пользователь видит только события 'all' / где он указан / где он в группе
- [x] Админ/наставник/суперадмин видят все события независимо от audience (для управления)
- [x] POST/PUT принимают `audienceType` + `audienceUserIds` / `audienceGroupIds`
- [x] При редактировании только админской выдачи приходит `audienceUserIds` / `audienceGroupIds` для предзаполнения формы
- [x] EventEditor — селектор "Все / Конкретные пользователи / По группам" + чекбокс-список с поиском
- [x] EventModal — у админа отображается строка "Видно: ..." с именами/группами

### 10.3. Уведомления по audience
- [x] Scheduler рассылает напоминания (1ч/5мин) только тем, кто входит в audience события
...
## Фаза 12. Программные чаты и автоматический доступ через группы

### 12.1. Программные чаты + переименование общего
- [x] Миграция `019_program_chats_and_group_tabs.sql` — колонка `chat_rooms.tab_slug` (UNIQUE среди general)
- [x] Переименовать вкладку и общий чат: `Беседа` → `Общая беседа` (slug `beseda`)
- [x] 4 новые вкладки: `beseda-arkhetipy`, `beseda-opora`, `beseda-otnosheniya`, `beseda-finansy` (sort_order 12–15)
- [x] 4 новые general-комнаты с фиксированными UUID и привязкой к вкладкам
- [x] Все текущие admin/mentor/superadmin автоматически добавлены в новые general-комнаты
- [x] Routes в `App.tsx` + иконки в `Sidebar.tsx` для новых slug-ов
- [x] `GeneralChatPage` сделан динамическим: roomId резолвится по `tabSlug` из URL через `GET /api/chat/rooms`
- [x] `seed.ts` идемпотентно создаёт новые вкладки + чаты + добавляет админов

### 12.2. Группы как пакеты доступа (group → tabs)
- [x] Таблица `user_group_tab_defaults` (group_id, tab_id) — какие вкладки открываются по умолчанию при добавлении в группу
- [x] Сидим 6 групп: `Клуб`, `Личка`, `Программа Архетипы/Опора/Отношения/Финансы` (с case-insensitive проверкой существующих)
- [x] Mapping группа → вкладки:
  - Клуб → Общая беседа, Марафоны, Лекции, Книги, Инструкции, Подкасты, Фильмы, Руководитель 360
  - Личка → Личная беседа, Марафоны, ДТП, Трекер созвонов
  - Программа Архетипы → Беседа Архетипы
  - Программа Опора → Беседа Опора
  - Программа Отношения → Беседа Отношения
  - Программа Финансы → Беседа Финансы
- [x] Backfill в миграции: уже существующим участникам "Клуб"/"Личка" автоматически открываются дефолтные вкладки + членство в чатах
- [x] Функция `applyGroupTabDefaults(userId)` — UPSERT в `user_tab_access` + `chat_room_members` (только добавляет, ничего не отзывает)
- [x] `PUT /api/users/:id/groups` вызывает `applyGroupTabDefaults` после изменения членства
- [x] `PUT /api/groups/:id/members` вызывает `applyGroupTabDefaults` для каждого участника
- [x] `PUT /api/users/:id/tabs` синхронизирует `chat_room_members` для general-чатов выданных вкладок
- [x] `addAdminToAllGeneralRooms` — при создании нового admin/mentor/superadmin он автоматически попадает во все general-комнаты
- [x] Старая чекбокс-матрица `user_tab_access` остаётся доступной — админ может тонко настраивать доступ поверх дефолтов

### 12.3. Прод-выкатка
- [x] Применить миграцию `019_program_chats_and_group_tabs.sql` на проде (`npm run migrate`)
- [x] (Опционально) запустить `npm run seed` — он идемпотентно подхватит группы и mapping, если суперадмин уже создан
- [ ] Проверить, что в админке появились 4 новые группы и 4 новые вкладки в матрице доступа
- [ ] Создать тестового пользователя и убедиться, что чекбоксы группы автоматически открывают перечисленные вкладки

---

## Фаза 13. Повторяющиеся события календаря

### 13.1. Backend
- [x] Миграция `020_recurring_events.sql` — `calendar_events.recurrence_id UUID`, `recurrence_pattern JSONB` + индекс
- [x] `POST /api/calendar/events` принимает `recurrence: { weekdays: [0..6], until: 'YYYY-MM-DD' }`
- [x] Генерация массива `start_at` по правилу: каждый день от `startAt` до `until`, у которого `getDay()` входит в `weekdays`
- [x] Все события серии получают общий `recurrence_id` (UUID) и копию `recurrence_pattern`
- [x] Защитный лимит `MAX_RECURRING_EVENTS = 365`
- [x] Возврат `{ events: [...], recurrenceId }` для серии, single event для разовых
- [x] `PUT /api/calendar/events/:id?scope=this|following` — затрагивает одну запись или все последующие в серии (по `start_at`)
- [x] При сдвиге даты с `scope=following` — все последующие сдвигаются на ту же дельту (сохранение расписания)
- [x] `DELETE /api/calendar/events/:id?scope=this|following` — soft-delete одной записи или текущей+всех последующих
- [x] Возврат `affectedCount` для UI

### 13.2. Frontend
- [x] `api/calendar.ts` — типы `RecurrencePattern`, `EditScope`, `CreateRecurringResult`; параметр `scope` в update/delete
- [x] `EventEditor` — блок "Повторять" (только при создании): чекбоксы дней недели Пн–Вс + поле "Повторять до"
- [x] Валидация: минимум 1 день, дата окончания не раньше начала
- [x] При редактировании события из серии — info-плашка о выборе scope
- [x] Компонент `RecurrenceScopeDialog` — диалог "Только это" / "Это и все последующие" с кнопкой Отмена
- [x] `CalendarPage`: при клике на Edit/Delete события с `recurrenceId` сначала показывается scope-диалог, затем уже открывается редактор / запрашивается подтверждение
- [x] Иконка ↻ возле названий повторяющихся событий (на карточке и в модалке)
- [x] Полный reload событий после изменений серии (когда затронуто > 1 записи)

### 13.3. Прод-выкатка
- [x] Применить миграцию `020_recurring_events.sql` на проде
